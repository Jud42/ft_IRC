...
void Server::Cmds_whois(int const fd_client, std::string const command)
{
	...
	if (position != std::string::npos) // if found
    	nick = nick.substr(0, position); // extract the string until there
	if (nick.size() == 0)
		nick = this->_fd_nick_list[fd_client];
	if (_fd_nick_list.count(nick) < 1) 
        std::cout << "Âge de Bob : " << mapExemple["Bob"] << std::endl;
    } else {
        std::cout << "Bob n'existe pas dans la map." << std::endl;
    }
	
	std::cout << "nick:" << nick << "*"<< std::endl;
	std::cout << YEL << "client: " <<  std::endl;
std::cout << this->_fd_nick_list[fd_client]  <<  std::endl;
	std::cout << this->_clientList[_fd_nick_list[fd_client]]->getNickname()  <<  std::endl;
